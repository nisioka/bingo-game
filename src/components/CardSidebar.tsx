...
const CardSidebar: React.FC = () => {
  const { bingoCards, cardCount, toggleCardAutoOpen } = useBingoStore();
    ...
    <div className="fixed top-1/2 transform -translate-y-1/2 right-2 z-40 flex flex-col space-y-3">
      {bingoCards.map(card => (
        <div key={card.id} className="relative flex flex-col items-center">
          ...
          {/* Per-card auto-open toggle */}
          <button
            onClick={() => toggleCardAutoOpen(card.id)}
            className={`mt-0.5 text-[9px] leading-none px-1 py-0.5 rounded font-bold border
              ${card.autoOpen
                ? 'bg-green-500 text-white border-green-600'
                : 'bg-white text-gray-500 border-gray-300 hover:bg-gray-100'}`}
            aria-pressed={card.autoOpen}
            aria-label={card.autoOpen ? 'このカードの自動表示をオフにする' : 'このカードを自動で開く'}
            title={card.autoOpen ? '自動で開く: オン' : '自動で開く: オフ'}
          >
            自動
          </button>
        </div>
